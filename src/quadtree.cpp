...
Node::Node(bool _is_leaf, Boundary _boundary, short int _luminance,
...
QuadTree::QuadTree(short int (*_IMG)[MAX_IMG_RES][MAX_IMG_RES], int _IMG_resolution, short int _compression_parameter)
{
    IMG = _IMG;
...
void QuadTree::construct_tree(Node* node)
{
    ...
    if (node->is_leaf)
    {
        ...
        push_short_to_binary_vector(node->luminance, &binarized_tree);
    }
...
void QuadTree::reconstruct_recursively(Boundary boundary, unsigned long* input_ctr)
{
    ...
    {
        (*input_ctr)++;
        short current_colour = read_short_from_binary_vector(binarized_tree, *input_ctr);
...
void QuadTree::write_binarized_tree_to_file(char* file_name)
{
    ...
    {
        current_byte = read_byte_from_binary_vector(binarized_tree, (byte_ctr * 8));
        fwrite(&current_byte, sizeof(uint8_t), 1, fout);
    }
...
void QuadTree::read_binarized_tree_from_file(char* file_name)
{
    ...
    {
        push_byte_to_binary_vector(current_byte, &binarized_tree);
    }
